...
- [x] `apps/pi-cli/README.md` install section now leads with `curl | bash`, then npm. (Only reaches the npm page on the next `npm publish`; the GitHub landing page shows the root `README.md`, which still has the old MVP notes and no install instructions.)
