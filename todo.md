...
- [x] **Bug in the trust gate as first released (v0.3.2, binary + npm): pressing `n` left the terminal broken.** Declining called `process.exit(0)` directly, which skips the renderer's shutdown — mouse tracking and raw mode stayed on, so the shell printed every mouse move as text (`35;65;7M35;65;7M…`) until the terminal was reset. Fixed by calling `renderer.destroy()` (via `useRenderer()`) before exiting. Reproduced first in a real pty by counting terminal-mode sequences (mouse-tracking/alt-screen ON at startup, but **zero** OFF sequences after `n`), then confirmed the fix emits all three OFF sequences, exits 0 immediately, and persists nothing. Also checked the other quit routes in a real pty — Ctrl+C and Esc on the gate, and Ctrl+C on the home screen — all already restore the terminal correctly (Ctrl+C is handled by the library). It got past my tests because the decline test stubbed `process.exit` and never checked *what happened before it*; the test now asserts the renderer is destroyed first, and I confirmed it fails when the bare `process.exit` is put back. **Needs a release (v0.3.3 — bump the three version files, commit, tag, and `npm publish`) — 0.3.2 with this bug is what's live.**
...
- [x] Directory trust gate (`packages/agent/trust.ts`) — every existing guard only fires *after* AEON starts operating; nothing asked permission to touch the folder at all, so `aeon` in any directory started reading files immediately. Now: on first launch in a directory (both `aeon run` and the TUI), AEON asks "AEON wants to read and edit files in: `<cwd>` — Trust this folder? [y/N]", and remembers the answer per-directory in `~/.pi-cli/trusted-dirs.json` (realpath'd, so symlinks to the same folder share one trust entry). Declining exits without calling `AgentCall` at all. Separate from `confirmTool` — this is a one-time "operate here at all" gate, not a per-action one; `write`/`edit`/`bash` still ask individually as before. Headless path uses blocking `readline` like the existing confirm prompt; the TUI needed a new `TrustGate` component + `useKeyboard` listener, since there's no blocking stdin once the TUI owns raw terminal input. 4 new tests in `App.test.tsx` (untrusted shows the gate, `y` trusts + persists + reveals home, `n` exits with nothing written, an already-trusted dir skips the gate next launch) plus the 3 CLI cases tested directly via subprocess (decline/approve/re-run). Found and fixed a real testing-infra bug along the way: Bun's `os.homedir()` snapshots `$HOME` once per process and ignores later writes to `process.env.HOME`, silently breaking test isolation — added an `AEON_HOME` override in `trust.ts` (a real, harmless config override, not just a test hook) and fixed a second bug where React's reconciler schedules re-renders through the real task queue, not microtasks, so `renderOnce()`/`waitForVisualIdle()` alone weren't enough to observe a state-driven UI change; needed an actual `setTimeout` yield.
...
- [x] Path (a) prepared and verified: package renamed `@ashuk971/nive` → **`aeon-ai`** (name confirmed free), bin `nive` → `aeon`, version 0.3.1, MIT, repo/keywords/engines filled in, `prepublishOnly` builds. Runtime deps trimmed to just `@opentui/core`, `@opentui/react`, `react` — everything else (`openai`, `@anthropic-ai/sdk`, `commander`) is inlined by the bundler, `axios` was entirely unused, and the workspace deps moved to devDependencies since `workspace:*` would break `npm i -g`. Published README written for npm users.
...
- [x] **v0.3.1 released, end to end verified.** Two things went wrong on the way, both worth remembering: (1) the first tag push fired no run — the workflow file and the tag arrived in the same push, and GitHub registered the workflow (13:35:40) after the commit (13:35:14) without evaluating the tag event against it; deleting and re-pushing the tag fixed it. (2) The first real run failed with `Could not resolve "commander"` — I'd dropped it from `apps/pi-cli/package.json` when slimming deps for npm, reasoning it's bundled into `dist` so not needed at runtime, but the bundler still needs it at *build* time; it only worked locally because stale installs left it in `node_modules`. Fixed by declaring it in devDependencies, and verified in a pristine copy with no `node_modules` under `--frozen-lockfile`. Run 2 passed in 27s: release `v0.3.1` (Latest) holds all 5 binaries. Then ran the real `curl … | bash` against it (into a temp dir via `AEON_INSTALL_DIR`) — installed a working 109 MB binary, `--version` = 0.3.1, TUI boots.
- [x] **Bug found after release: v0.3.0 (binary *and* npm `aeon-ai@0.3.0`) crashed in any real terminal** — `unable to determine transport target for "pino-pretty"`, even on `aeon --help`. `logger.ts` used pino's `transport: { target: "pino-pretty" }`, which loads the module by name in a worker thread at runtime; no bundler or `bun build --compile` can see that, and `pino-pretty` isn't a runtime dep of the published package, so it only worked in the repo where `node_modules` exists. It was invisible to every test I ran because the pretty-print branch only executes when `stderr.isTTY`, and all my runs had output piped — so "verified working" was wrong for real terminals. Fixed by importing `pino-pretty` as a normal module and passing it to pino as a stream (bundler includes it, no worker thread). Verified under a pseudo-terminal (`script -qec`): `--help`, colored log output in a TTY, JSON when piped, TUI boot, node bundle, and the packed npm tarball installed into a clean dir; plus the fix built from a pristine copy under `--frozen-lockfile`. **Status: npm `aeon-ai@0.3.1` is published and verified working in a terminal (installed the real published tarball, ran under a pty). The binary release is NOT fixed yet** — the fix commit (`c7aa74c`, version 0.3.1) is on master, but the only tag is `v0.3.0`, still pointing at the pre-fix commit `8bd6cd0`. Re-pushing that tag re-ran the workflow on the *same old commit* and republished the same broken binaries (confirmed: installing from the live release gives `--version` 0.3.0 and the pino crash). Needs a **new tag `v0.3.1` at HEAD** — a tag names a commit, so re-pushing a tag never picks up new code. (Careful with global find-and-replace on version numbers: it rewrites history in notes like this one.)** **It also broke CI:** the same `0.3.0` → `0.3.1` replace hit `bun.lock`, turning third-party entries into `groq-sdk@0.3.1` (still carrying the 0.3.0 checksum) plus four unrelated dependency ranges (`^0.3.0`→`^0.3.1`, `^10.3.0`→`^10.3.1`, …). CI failed in `bun install` with `IntegrityCheckFailed for groq-sdk` — bun was right, the lock entry and its hash disagreed (confirmed by hashing the real tarball). Fixed by restoring `bun.lock` from the last passing commit and re-applying only the legitimate line (the `aeon-ai` workspace version). Only edit the version in `apps/pi-cli/package.json`, `src/index.ts`, and `packages/tui/src/env.ts` — never the lockfile.
- [x] CI smoke test now runs under a pty (`script -qec … | grep "Usage: aeon"`). The old one ran `--help` with captured output — not a TTY — so it could never have caught this. Proved the new check fails on the old logger and passes on the fix.
- [ ] Version is hardcoded in three places (`apps/pi-cli/package.json`, `.version(...)` in `apps/pi-cli/src/index.ts`, `VERSION` in `packages/tui/src/env.ts`) — easy to bump one and forget another.
- [ ] While the TUI is open, anything the logger writes to stderr lands on the same terminal the UI is drawing on. Default level is `info` and the code mostly logs at `debug`, so it rarely shows, but a retry/compaction `warn` would corrupt the screen. Worth routing logs to a file in TUI mode.
- [x] `apps/pi-cli/README.md` install section now leads with `curl | bash`, then npm. (That one only reaches the npm page on the next `npm publish`. The GitHub landing page shows the *root* `README.md`, which had no install instructions at all until now — root README now opens with the AEON title, `curl | bash` + npm install, and a quick start, with the old MVP notes kept below.)
- [ ] Not verified: the darwin and windows binaries have never been *run* — only built. Everything I confirmed by execution was linux-x64. A mac user is the first real test of `aeon-darwin-*`. Windows can't use `install.sh` at all (it exits with a pointer to npm).
- [ ] Note: the install command is pinned to the `master` branch, so it always serves whatever `install.sh` is on master, while `releases/latest` moves independently — the script and the binary can drift apart if `install.sh` changes in a way old releases don't support.
